...
public class IndexActivity extends Activity {
	...
	protected void onActivityResult(int requestCode, int resultCode, Intent intent) {
		...
		try {
			accessToken = twitterOauth.getOAuthAccessToken(requestToken,
					intent.getExtras().getString(TwitterConst.PARAM_OAUTH_VERIFIER));
	...
	private void tweet() {
		...
		try {
			twitter.updateStatus("自作クライアントからツイットのテスト");
		} catch (TwitterException e) {
			e.printStackTrace();
		}
	}
